...
public class TileTool2D : EditorWindow {
	...
	public void EnableSceneGUI(bool enabled) {
		//Debug.Log("EnableSceneGUI " + enabled);
	...
	public void ChangeCanvas() {
		if(CanvasContainer.Length > 0) layerHolder = CanvasContainer[canvaseIndex].gameObject;
	}
	...
	public void OnGUI() {
		...
		canvaseIndex = EditorGUILayout.Popup(canvaseIndex, canvasSets, GUILayout.Width(Mathf.Min(position.width * 0.48f, 250)));
		...
		for (int i = 0; i < tiles.Length; i++) {
			...
			Sprite selectedSprite = tiles[i].GetComponent<SpriteRenderer>().sprite;
		...
		if(drawTile) GUILayout.Label("" + drawTile.name, GUILayout.Height(16));
		EditorGUILayout.BeginHorizontal();
